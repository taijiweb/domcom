...
* cdn(x.y.z使用实际的版本号，比如0.8.0): `http://cdn.jsdelivr.net/domcom/x.y.z/domcom.min.js`
...
部件辅助函数：`isBlock`, `toBlock`, `toBlockArray`
...
### List部件（列表） 
    new List(children)
	 list(attrs:Attrs, children...:[toBlock])
    ...
    如果items是数组，则itemFn的类型标签为`(item, index, items，component) -> toBlock`
    如果items是数组，则itemFn的类型标签为`(value, key, index, items，component) -> toBlock`
